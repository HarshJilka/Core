...
<select id="selectcustomer" onchange="changeCustomer()">
    ...
    <?php foreach($customers as $cust): print_r($cust->customerId); ?>

    <option value="<?php echo $cust->customerId ?>" <?php echo ($cust->customerId == $customer->customerId)?'selected':'' ?>>
    ID : <?php echo $cust->customerId ?>
    Name : <?php echo $cust->firstName." ".$cust->lastName." Email: ".$cust->email; ?></option>
        ...
        <tr>
            <td>
                <table border="1">
                    ...
                    <tr>
                        <td colspan="2"><input type="checkbox" name="saveToBillingBook" value=1>SAVE TO ADDRESSBOOK</td>
                    </tr>

                    <tr>
                        <td colspan="2"><input type="checkbox" name="copyAddress" id="copyAddress" onchange="sameAddress()" />MARK AS SHIPPING</td>
                    </tr>
                </table>
            </td>
            ...
            <td>
                <table border="1">
                    ...
                    <tr>
                        <td colspan="2"><input type="checkbox" name="saveToShippingBook">SAVE TO ADDRESSBOOK</td>
                    </tr>
                </table>
            </td>
        </tr>
...
<table border="1" align="center">
    ...
    <tr>
            ...
            <table>
                <form action="<?php echo $this->getUrl('savePaymentMethod');?>" method="POST">
                    <?php foreach ($paymentMethods as $paymentMethod) : ?>
                        <tr>
                            <td><input type="radio" name="paymentMethod" value="<?php echo $paymentMethod->methodId ?>" <?php echo ($paymentMethod->methodId == $cart->paymentMethod)?'checked' : '' ?>><?php echo $paymentMethod->name ?>
                            </td>
                        </tr>
                    <?php endforeach;?>
                    <tr>
        ...
        <td>
            <table>
                <form action="<?php echo $this->getUrl('saveShipingMethod');?>" method="POST" align="center">
...
<br>
<br>
<div id="productTable" align="center">
    ...
    <form action="<?php echo $this->getUrl('cartItemUpdate'); ?>" method="POST">
        ...
        <table border="1">
            ...
            <tr>
                ...
                <td>
                    <img src="<?php echo $item->getProduct()->getBase()->getImgPath(); ?>" alt="No Image Found" width="50" height="50">
                </td>
                <td><?php echo $item->getProduct()->name; ?></td>
                <td>
                    <input type="number" name="cartItem[<?php echo $i ?>][quantity]" value="<?php echo $item->quantity; ?>" min="1">
                </td>
                <td><?php echo $item->getProduct()->price; ?></td>
                <td align="right"><?php echo $item->itemTotal; ?></td>
                <td align="center">
                    <a href="<?php echo $this->getUrl('deleteCartItem',null,['itemId' => $item->itemId]) ?>">Remove</a>
                </td>
            </tr>
            <?php $i++; ?>
            <?php endforeach; ?>
...
<form action="<?php echo $this->getUrl('placeOrder') ?>" method="POST">
    <table align="center">
        ...
        <tr>
            ...
            <td>
                <?php echo $this->getTotal() + ($cart->shippingCharge) + $this->getTax($cart->cartId) - $cart->discount;; ?>
            </td>         
        </tr>
